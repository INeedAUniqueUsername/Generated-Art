...
public class RadiantRing {
	public static void main(String[] args) {
		Ring();
	}
	public static void Ring() {
		try {
			GifSequenceWriter capture = Help.createGifWriter("./RadiantRing");
			
			int width = 1024; //256
			int height = 1024; //256
			RadiantRing effect = new RadiantRing(width, height);
			int i = 0;
			while(effect.moreFramesLeft()) {
				...
				effect.paint(g);
				ImageIO.write(img, "png", new File("./RadiantRing/Color/" + i + ".png"));
				
				ImageIO.write(Help.createMask(img), "bmp", new File("./RadiantRing/Mask/" + i + ".bmp"));
	...
	public RadiantRing(int width, int height) {
		this.width = width;
		this.height = height;
	}
